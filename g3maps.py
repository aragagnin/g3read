...
@jit(nopython=True)
def     add_to_grid(final_image_t, masked_x, masked_y, masked_h, masked_w, masked_m, bin_min, delta_bin, n_bins, sph, mass_w):
    ...
    N = len(masked_x)
    final_image_count = final_image_t.copy()
        ...
        if sph:
            for i in range(bin_i - max_bin_spread, bin_i + max_bin_spread + 1):
                ...
                for j in range(bin_j - max_bin_spread, bin_j + max_bin_spread + 1):
                    ...
                    final_image_t[bin_j][bin_i]+=v
                    final_image_count[bin_j][bin_i]+=masked_m[k]

    if mass_w:
        final_image_t[:,:]/=final_image_count
    if N>0:
        return tot_max_bin_spread/N
        #return tot_max_bin_spread/
    else:
        return np.nan


def smac_gen_image_matrix(img_size):
    n_bins=[img_size]*2
    final_image_t = np.zeros(n_bins)
    return final_image_t

def smac_add_chunk(P, ptype, gpos, axis_z, img_xy_size, img_z_size, img_size, PROPERTY, result_units, final_image_t, sph=True, image_w=False):
    ...
    masked_w = (P[PROPERTY][mask]).to(result_units).magnitude
    masked_m = (P['MASS'][mask]).to('gmass').magnitude
    ...
    print('goin to addcrid')

    avg_bin_spread = add_to_grid(final_image_t, masked_x, masked_y, masked_h, masked_w, masked_m, bins[0][0], bins[0][1]-bins[0][0], img_size, sph, image_w)
    print ('# avg bin spread ', avg_bin_spread)
    return avg_bin_spread
...
def smac_for_poors(kv):
    ...
    final_image_t = smac_gen_image_matrix(img_size)
    ...
    for _res in  g3u.yield_particles_blocks_in_box(kv['SNAP_PATH'], gpos_cu, max_xyz_cu,[PROPERTY,'POS ','RHO '], ptypes,debug=True, units=units):
        for ptype in _res:
            smac_add_chunk(_res[ptype], ptype, gpos, 2, img_xy_size, img_z_size, img_size, PROPERTY, kv['RESULT_UNITS'], final_image_y)
